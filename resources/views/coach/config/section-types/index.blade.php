<x-app-layout>
    <div class="max-w-5xl mx-auto px-4 py-6">
        <div class="flex items-start justify-between gap-4">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900">Tipos de seccion</h1>
                <p class="text-sm text-gray-600">Administra los bloques disponibles para estructurar tus entrenamientos.</p>
            </div>

            <a href="{{ route('coach.config.section-types.create') }}"
                ...
                <table class="min-w-full text-sm">
                    <thead class="bg-slate-50 text-slate-700">
                        <tr>
                            ...
                            <th class="text-left font-medium px-4 py-3">Descripcion</th>
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('coach.config.section-types.edit', $item) }}"
                                           class="px-3 py-1.5 rounded-lg border text-xs hover:bg-gray-50 transition">
                                            Editar
                                        </a>

                                        <form action="{{ route('coach.config.section-types.destroy', $item) }}" method="POST"
                                              onsubmit="return confirm('Eliminar tipo de seccion?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-4 py-10 text-center text-slate-500">
                                    No hay tipos de seccion todavia.
                                    <a href="{{ route('coach.config.section-types.create') }}" class="text-slate-900 underline">
                                        Crea el primero
                                    </a>.
                                </td>
                            </tr>
